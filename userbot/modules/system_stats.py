...
@register(outgoing=True, pattern=r"^\.(?:nomercyalive)\s?(.)?")
async def amireallyalive(alive):
    user = await bot.get_me()
    await get_readable_time((time.time() - StartTime))
    output = (
        f"\n__**{NOMERCY_TEKS_KUSTOM}**__\n"
        f"**╭───────────────────**\n"
        f"**├[• 🤴 Nomercy Owner** \n"
        f"├[•   : `{DEFAULTUSER}` \n"
        f"**├[• 🔎 Username** \n"
...
@register(outgoing=True, pattern=r"^\.(?:nmrcy)\s?(.)?")
async def amireallyalive(alive):
    await bot.get_me()
    await get_readable_time((time.time() - StartTime))
    output = (
        f"**ㅤㅤㅤㅤ 🔥𝗡𝗼𝗺𝗲𝗿𝗰𝘆-𝙐𝙨𝙚𝙧𝙗𝙤𝙩🔥**\n"
        f"╔══════════╬════╬══════════╗ \n"
        f"╟⟩⟩ 🤴 • `ᴏᴡɴᴇʀ    :`[N O M E R C Y](t.me/xenxust)             ㅤ ║\n"
        ...
        f"╚══════════╬════╬══════════╝ \n"
        f"🔥 : [𝗥𝗲𝗽𝗼-𝗡𝗢𝗠𝗘𝗥𝗖𝗬](https://github.com/VickyMIPA/Nomercy-UserBot)\n👥 : [GROUP](t.me/SocializeLives)\n👤 : [𝗢𝗪𝗡𝗘𝗥](t.me/xenxust)\n"
    )
    if ALIVE_LOGO:
        try:
            logo = ALIVE_LOGO
...
async def redis(alive):
    ...
    output = (
        ...
        f"╭═───────╼⌘╾───────═ \n"
        f"┃[𝗥𝗲𝗽𝗼](https://github.com/Nomercy-UserBot)  |  [𝗦𝘂𝗽𝗽𝗼𝗿𝘁](t.me/SocializeLives)  |  "
...
CMD_HELP.update({
    "alive":
    "𝘾𝙤𝙢𝙢𝙖𝙣𝙙: `.alive` or `.on` or `.nomercyalive`"
